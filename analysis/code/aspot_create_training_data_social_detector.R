...
for(type in c('target', 'noise')){
  ...
  # Paths 
  path_selections = 
    sprintf('analysis/data/social_detector/training_data/%s', type)
  path_wavs = 'analysis/data/audio'
  path_results = 
    sprintf('aspot/data_sets_social_detector/data_%s/data', data_set)
  path_aug = sprintf('aspot/data_sets_social_detector/data_%s/aug_data', 
                     data_set)
