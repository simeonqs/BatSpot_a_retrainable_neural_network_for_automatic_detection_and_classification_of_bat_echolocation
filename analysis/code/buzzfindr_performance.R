# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Project: buzzfindr bats  
# Author: Simeon Q. Smeele
# Description: Plots confusion matrix for buzz detector.
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# Paths
path_buzzfindr = 'buzzfindr/detected_buzzes.csv'
path_ground_truth = 'analysis/data/buzz_detector/validation_data/ground_truth'
path_pdf = paste0('analysis/results/buzzfindr/confusion_matrix.pdf')

# Load data
buzzfindr = read.csv(path_buzzfindr)
buzzfindr$Sound.type = 'target'
buzzfindr$Begin.time..s. = buzzfindr$tstart
buzzfindr$End.time..s. = buzzfindr$tstart + 0.2
buzzfindr$file = buzzfindr$file |> str_remove('.wav')
...
# Test if all files in are in the manual and buzzfindr data frame
files_gt = buzzfindr$file
files_bf = list.files(path_buzzfindr, recursive = TRUE) |> 
  basename() |> 
  str_remove('_predict_output.log.annotation.result.txt')
if(any(!files_bf %in% files_gt)) stop('Missing ground truth.')
...
for(file in files){
  ...
  file = file |> 
    str_remove('.Table.1.selections.txt') |>
    str_remove('.Band.Limited.Energy.Detector.selections.txt')
  d = buzzfindr[buzzfindr$file == file,]
...
if(length(fps) > 0)
  class_results = rbind(class_results,
                        data.frame(file = buzzfindr[fps,]$file,
                                   row_d = fps,
                                   row_g = NA,
                                   d = buzzfindr[fps,]$Sound.type,
                                   g = 'noise',
                                   iou = NA))
...
if(any(!rownames(buzzfindr) %in% class_results$row_d)) 
  stop('Missing rows from buzzfindr.')
